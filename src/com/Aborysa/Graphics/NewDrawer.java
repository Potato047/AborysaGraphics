...
public class NewDrawer {
	...
	public static void drawImagePart(int x, int y,int xp1,int yp1, int xp2, int yp2, BufferedImage image){
		if (image.getRaster().getDataBuffer() instanceof DataBufferByte){
			...
			drawPixelArray(x,y,xp1,yp1,xp2,yp2,image.getWidth(),pixels);
		}else if (image.getRaster().getDataBuffer() instanceof DataBufferInt){
			int[] pixels = ((DataBufferInt)image.getRaster().getDataBuffer()).getData();
			drawPixelArray(x,y,xp1,yp1,xp2,yp2,image.getWidth(),pixels);
		}		
	...
	public static void drawPixelArray(int x, int y,int xp1, int yp1,int xp2,int yp2, int width, int[] pixelArray){
		if(imageDraw){
			int[] pixels = ((DataBufferInt)imageTarget.getRaster().getDataBuffer()).getData();
			for(int i = (xp1+yp1*width); i< (xp2 + yp2 * width);i++){
			...
			NewDrawer.drawPixelArray(x, y,xp1,yp1,xp2,yp2, width, pixelArray);
	...
	public static void drawBuffer(int x, int y ,DrawingBuffer2 buffer){
		if(bufferDraw2){
			...
			drawPixelArray(x,y,0,0,buffer.getWidth(),buffer.getPixelCount()/buffer.getWidth(),buffer.getWidth(),pixels);	
		}
		if(canvasDraw){
	...
	public static void drawPixelArray(int x, int y,int xp1,int yp1 ,int xp2, int yp2, int width, byte[] pixelArray){
		if(imageDraw){
			int[] pixels = ((DataBufferInt)imageTarget.getRaster().getDataBuffer()).getData();
